...
def add_me_with_my_friends(queue, index, person_name):
    ...
    if index > len(queue):
        raise ValueError("Adding the new person_name in the provided position (specified by index) is only possible if this index is not bigger than the initial queue length, in which case the provided position does not exist in this queue.")

    # if index == 0:
    #     queue = [person_name] + queue     
    # elif index == len(queue):
    #     queue = queue + [person_name]
    #     # queue.append(person_name)
    # else:
    #     first_part = queue[:index]
    #     last_part = queue[index:]
    #     # queue = queue[:index] + [person_name] + queue[index:]
    #     queue = first_part + [person_name] + last_part
...
def remove_the_last_person(queue):
    ...
    return queue.pop(-1) # same as queue.pop(len(queue))
