...
def extend_list(acc, extension_list):
    acc.extend(extension_list)
    return acc
...
class School:
    ...
    def roster(self):
        return reduce(extend_list, # external_binary_operation_defining_function
                  (sorted(self._students_by_grade[g]) for g in sorted(self._students_by_grade.keys())), # current_iteration_operand
                  []) # starting_point
